...
const toneStyles: Record<ToastTone, { bg: string; icon: JSX.Element }> = {
  default: {
    bg: "bg-ink-900",
    icon: <Info size={18} weight="bold" className="text-surface" />,
  },
  success: {
    bg: "bg-ink-900",
...
export const Toaster = memo(function Toaster() {
  ...
  return (
    <div className="pointer-events-none fixed left-0 right-[8vw] bottom-0 z-50 flex flex-col items-center gap-2 pl-5 pr-2 pb-[max(1.25rem,env(safe-area-inset-bottom))] md:inset-x-0 md:px-4">
      <AnimatePresence initial={false}>
        {toasts.map((t) => (
          ...
          >
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-surface/10">
              {toneStyles[t.tone].icon}
            </span>
            <span className="flex-1 text-sm font-medium text-surface">{t.message}</span>
          </motion.button>
        ))}
      </AnimatePresence>
    </div>
